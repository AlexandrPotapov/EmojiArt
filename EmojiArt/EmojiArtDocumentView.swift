...
//  EmojiArt
//
//  Created by Александр on 04.08.2022.
//
...
struct EmojiArtDocumentView: View {
    ...
    var body: some View {                                      // Main View
        VStack(spacing: 0) {
            ZStack(alignment: .topLeading){
                documentBody
                optionalRemoveButton.padding(.vertical).transition(.opacity)
            }
    ...
    @ViewBuilder
    private var optionalRemoveButton: some View {               // optional delete button popup
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                OptionalImage(uiImage: document.backgroundImage)
                    ...
                    .gesture(doubleTapToFit(in: geometry.size).exclusively(before: singleTapToDeselect()))
                if document.backgroundImageFetchStatus == .fetching {
                    ProgressView().scaleEffect(2)
                } else {
                    ForEach(document.emojis) { emoji in
                        Text(emoji.text)
                            .border(Color.black, width: selectedEmojis.containMatching(emoji) ? 3 : 0)
                            .font(.system(size: fontSize(for: emoji)))
                            .scaleEffect(emojiScale(for: emoji))
                            .scaleEffect(zoomScale)
                            .offset(selectedEmojis.containMatching(emoji) ? gestureEmojiPanOffSet: .zero)
    ...
    private func emojiScale (for emoji: EmojiArtModel.Emoji) -> CGFloat {
        selectedEmojis.containMatching(emoji) ? (zoomScale * gestureEmojiZoomScale) : zoomScale
    }
    ...
    private func panGesture(for emoji: EmojiArtModel.Emoji? = nil) -> some Gesture {
        if let emoji = emoji, selectedEmojis.containMatching(emoji) {
            return DragGesture()
